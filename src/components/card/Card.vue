...
import ResearchPseudoButton from '../button/PseudoButton.vue'
...
onMounted(() => {
  if (props.url) {
    applyClickA11y('.click-container:not([data-research-no-link])');
  }
});
  ...
  <div :class="classes">
    ...
    <div class="card__body">
      ...
      <footer v-if="url && (link_indicator || link_text)">
        <research-button :class="buttonClasses" :url="url" size="medium" v-if="linkedElement === 'button'">
          {{ link_text }}
        </research-button>
        <research-pseudo-button :class="buttonClasses" v-else>{{ link_text }}</research-pseudo-button>
      </footer>
    </div>
  </div>
</template>
